...
def process_raw_opportunity(self=None, raw_id: int = 0, model: str | None = None) -> dict:
    """
...
def process_pending_raw(self=None, limit: int = 25) -> dict:
    """
    ...
    for rid in ids:
        if getattr(settings, "CELERY_ENABLED", True):
            ...
            # Run inline (cron/HTTP triggered)
            process_raw_opportunity(raw_id=rid)
        processed += 1

    return {"processed": processed, "limit": limit, "mode": "celery" if getattr(settings, "CELERY_ENABLED", True) else "inline"}
