...
class RawOpportunityExtractor:
    ...
    def _ensure_english_text(self, raw: RawOpportunity, *, model: str | None = None) -> str:
        ...
        res = provider.translate_to_english(text=raw_text, model=model)
    ...
    def _validate_taxonomy_ids(self, data: dict) -> tuple[OpportunityType, Domain, Specialization, Location | None]:
        op_type = OpportunityType.objects.get(id=int(data["op_type_id"]))
        domain = Domain.objects.get(id=int(data["domain_id"]))
        spec = Specialization.objects.get(id=int(data["specialization_id"]))
        ...
        try:
            ...
            if raw.content_hash:
                ...
                if prev_raw and prev_opp:
                    ...
                    return ExtractResult(created=True, opportunity_id=opp.id)

            provider = get_provider()
            prompt = build_extract_prompt(text_en=text_en, source_url=raw.source_url or "")
            ...
            # Validate taxonomy consistency and load instances
            ...
            raw.status = RawOpportunity.ProcessingStatus.FAILED
            raw.error_message = str(e)[:2000]
            raw.save(update_fields=["status", "error_message"])
            raise
